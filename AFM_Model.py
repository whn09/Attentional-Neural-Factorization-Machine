    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # 默认使用cpu:
            ...
            self.nonzero_embeddings = tf.nn.embedding_lookup(self.weights['feature_embeddings'],
                                                             self.train_features)  # None * M' * K; M'即fields, K即em_factor
            ...
            if self.attention:
                self.attention_mul = tf.reshape(
                    tf.matmul(tf.reshape(self.element_wise_product, shape=[-1, self.em_factor]), \
            ...
            if self.lamde_em > 0:
                self.loss = self.loss + tf.contrib.layers.l2_regularizer(self.lamda_em)(
    ...
    def _initialize_weights(self):
        ...
        if self.attention:
            ...
            all_weights['attention_W'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.em_factor, self.attention_factor)), dtype=np.float32,
                name="attention_W")  # K * AK
            all_weights['attention_b'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(1, self.attention_factor)), dtype=np.float32,
                name="attention_b")  # 1 * AK
            all_weights['attention_p'] = tf.Variable(
                np.random.normal(loc=0, scale=1, size=(self.attention_factor)), dtype=np.float32,
                name="attention_p")  # AK
    ...
    def evaluate(self, data):  # 评价给定的数据集
        ...
        while len(batch_xs['X']) > 0:
            ...
            feed_dict = {self.train_features: batch_xs['X'], self.train_labels: [[y] for y in batch_xs['Y']],
                         self.dropout_keep: [1], self.train_phase: False}
            a_exp, a_sum, a_out, batch_out = self.sess.run(
                (self.attention_exp, self.attention_sum, self.attention_out, self.out), feed_dict=feed_dict)
